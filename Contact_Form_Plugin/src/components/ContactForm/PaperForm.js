...
const PaperForm = (props) => {
  ...
  const { isLoading: isLoadingMutation, mutate: mutate_Lead_Listing } =
    useMutationHelper({
  ...
  return (
    ...
    >
      {onSuccessAlert ? (
        <>
          <Text component="h3" className={styles.titlePaper}>
            Let’s get in touch:
          </Text>
          <Box className="flex justify-center w-full h-full mt-12">
            <Text component="p" className={styles.descriptionForm}>
              Thank you for submitting a request for information one of our
              agents will be in contact with you shortly
            </Text>
          </Box>
        </>
      ) : (
        <>
          ...
          {listingData ? (
            <Text component="p" className={styles.descriptionForm}>
              Complete this form to get additional information on{" "}
              <span className="text-[#ffb839]">
                {listingData?.title || "this property"}.
              </span>
            </Text>
          ) : (
            <Text component="p" className={styles.descriptionForm}>
              Using the form below, please provide as much detailed information
              as possible.
            </Text>
          )}
