...
// mantine
import { Box, Text, Badge, Divider, Tooltip, Textarea } from '@mantine/core';
...
const FinalStepp = () => {
  ...
  const statusUserLead = addLeads.stateLeads
    ? global.statusUserLead.find((item) => item.value === addLeads.stateLeads)
        .label
    : 'Not Contacted';
  ...
  return (
    <>
      {agentType === USER_ROLES_CRM.ADMIN ? (
        ...
        >
          <Text className={classes.titleFinalStepIfo} component='h3'>
            Selected agent
          </Text>
      ...
      <Box className={classes.containerFinalSteps}>
        <Box className={`${classes.contedInterested} ${classes.contendInfo}`}>
          ...
          <Box className={classes.containerInfo}>
            <Text component='span'>
              <strong>Status Leads:</strong>
            </Text>
            <Tooltip position='right' withArrow label={statusUserLead}>
              <Badge
                style={{ width: '103px' }}
                size='lg'
                variant='outline'
