...
const TransferAgent = forwardRef(
  ({ textTitle, data, checkList, checkAgent, omit }, _ref) => {
    ...
    return (
      <Box className={classes.container}>
        ...
        <Box
          ref={refParentBox}
          className={cx("parentContainerInfinite", classes.virtualContainer)}
        >
          <Grid
            ...
            {({ rowIndex, style }) => {
              return (
                ...
                >
                  <Box
                    onClick={() =>
                      checkAgent(dataForVirtualList[rowIndex]?.value)
                    }
                    className={classes.containerAgent}
                  >
                    <AvatarText src={dataForVirtualList[rowIndex]?.image} />
                    <Box className={classes.infoAgent}>
                      <Text component="span">
                        {dataForVirtualList[rowIndex]?.label}
                      </Text>
                      <Text component="span">
                        {dataForVirtualList[rowIndex]?.email
                          ? dataForVirtualList[rowIndex]?.email.toLowerCase()
                          : ""}
                      </Text>
                    </Box>
                    <Checkbox
                      defaultChecked={
                        findIndex(
                          checkList,
                          (e) => e === dataForVirtualList[rowIndex]?.value
                        ) > -1
                      }
                    />
                  </Box>
