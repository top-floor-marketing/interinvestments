...
const LeadDetailView = () => {
  ...
  useEffect(() => {
    ...
    if (agentType === USER_ROLES_CRM.ADMIN && infoInLocalStorage) {
      const validAgents = infoInLocalStorage?.allAgents.filter(
        (e) => e?.id > 0
      );
      setVerifyLocalStorage({
        ...infoInLocalStorage,
        allAgents: validAgents ? validAgents : [],
      });
    } else {
  ...
  if (agentType === USER_ROLES_CRM.ADMIN && verifyLocalStorage) {
        ...
        {(verifyLocalStorage?.allAgents).map((e, index) => (
          ...
          >
            <DetailLeadByAgent
              ...
              idAgent={e.id}
              idLead={verifyLocalStorage?.idLead}
            />
          </Tabs.Panel>
        ))}
      </Tabs>
    ...
    return (
      <DetailLeadByAgent
        idAgent={verifyLocalStorage?.idAgent}
        idLead={verifyLocalStorage?.idLead}
      />
    );
  }
