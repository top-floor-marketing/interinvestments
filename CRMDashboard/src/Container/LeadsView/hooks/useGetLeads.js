...
import {
  GET_LISTINGS_CATEGORY,
  GET_ALL_NEIGHBORHOODS,
  GET_ALL_LISTINGS,
} from "../../../GraphqlClient/listings.gql";
import {
  GET_AGENT_FEATURED_LISTING,
  MUTATION_ADD_AGENT_LISTING,
  MUTATION_DETELE_AGENT_LISTING,
} from "../../../GraphqlClient/agentProfile.gql";
...
const useGetListings = () => {
  const {
    ...
    actions: { setListingCategories, setListingNei, setListingFeaturedAgent },
  ...
  const {
    isSuccess: isSuccesFeatured,
    isError: isErrorFeatured,
    refetch: refetchAllFeatured,
  } = useQueryHelper({
  ...
  return {
    isSkeleton:
      isLoadingListing && (!categorySelect || !neiSelect || !isSuccesFeatured),
    isError:
      isErrorNei || isErrorCategory || isErrorFeatured || isErrorAllListings,
    categoryProps: {
