...
const FormEditProfile = ({ isLoading, dataAgent, onSubmit }) => {
  ...
  const form = useForm({
    ...
    initialValues: {
      content: removeTags(get(dataAgent, ['content'], '')),
  ...
  const onChangeAvatar = async (e) => {
    try {
      ...
      if (
        typeImagen === 'image/png' ||
        typeImagen === 'image/jpeg' ||
        typeImagen === 'image/jpg'
      ) {
        setnewAvatarFile(e);
  ...
  return (
    <>
      <form
        ...
        onSubmit={form.onSubmit((values) => validationSumitForm(values))}
      >
        <ScrollArea offsetScrollbars scrollbarSize={6}>
          <Box className={classes.containerForm}>
            <AvatarInput
              urlImagen={tempAvatar64 || get(dataAgent, ['avatarProfile'], '')}
            ...
            <Box className={classes.gridColumnFull}>
              <Text
                component='label'
                htmlFor='content'
                className={classes.labelAboutMe}
              >
                About me
              </Text>
              <Textarea
