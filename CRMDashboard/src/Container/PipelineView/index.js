...
const Pipeline = () => {
  ...
  const getInfoState = useCallback(
    (label) => {
      return get(
        filter(listStatus, (val) => toLower(val.label) === toLower(label)),
        ["0", "value"],
        0
      );
    },
  ...
  const {
    data: dataAllPipeline,
    isLoading: isLoadingAll,
    refetch: refecthAll,
  } = useGetAdminPipeline({ agentType });
  ...
  const {
    ...
    refetch: reFetchNotContacted,
    isLoading: isLoadingNotContacted,
  } = useGetPipelineLeads({
    agentId: getIdForPipeline(),
    agentSelected: idAgentForAdmin,
    statusId: getInfoState(PIPELINE_STATUS.NOT_CONTACTED),
  });
  const {
    data: dataContacted,
    ...
    agentId: getIdForPipeline(),
    agentSelected: idAgentForAdmin,
    statusId: getInfoState(PIPELINE_STATUS.CONTACTED),
  });
  const {
    data: dataShowing,
    refetch: reFetchShowing,
    isLoading: isLoadingShowing,
  } = useGetPipelineLeads({
    agentId: getIdForPipeline(),
    agentSelected: idAgentForAdmin,
    statusId: getInfoState(PIPELINE_STATUS.SHOWING),
  });
  const {
    data: dataContract,
    ...
    agentId: getIdForPipeline(),
    agentSelected: idAgentForAdmin,
    statusId: getInfoState(PIPELINE_STATUS.CONTRACT),
  });
  const {
    data: dataASk,
    refetch: reFetchAsk,
    ...
    agentId: getIdForPipeline(),
    agentSelected: idAgentForAdmin,
    statusId: getInfoState(PIPELINE_STATUS.ASK_REFERRALS),
  });

  const refechPipeline = (prevState, newState) => {
    if (
      getInfoState(PIPELINE_STATUS.NOT_CONTACTED) === prevState ||
      getInfoState(PIPELINE_STATUS.NOT_CONTACTED) === newState
    )
      reFetchNotContacted();
    if (
      getInfoState(PIPELINE_STATUS.CONTACTED) === prevState ||
      getInfoState(PIPELINE_STATUS.CONTACTED) === newState
    )
      reFetchContacted();
    if (
      getInfoState(PIPELINE_STATUS.SHOWING) === prevState ||
      getInfoState(PIPELINE_STATUS.SHOWING) === newState
    )
      reFetchShowing();
    if (
      getInfoState(PIPELINE_STATUS.CONTRACT) === prevState ||
      getInfoState(PIPELINE_STATUS.CONTRACT) === newState
    )
      reFetchContract();
    if (
      getInfoState(PIPELINE_STATUS.ASK_REFERRALS) === prevState ||
      getInfoState(PIPELINE_STATUS.ASK_REFERRALS) === newState
    )
      reFetchAsk();
  };

  const USING_FULL_PIPELINE =
    agentType === USER_ROLES_CRM.ADMIN && !idAgentForAdmin;
    ...
    <div style={{ width: "100%", position: "relative" }}>
      ...
      <Box className={classes.container}>
        {agentType === USER_ROLES_CRM.ADMIN && (
          <PaperFilterAgent
            idAgent={idAgentForAdmin}
            refecthAll={refecthAll}
            setIdAgent={setIdAgentForAdmin}
          />
        )}
        ...
        <Box className={classes.containerPipeline} ref={ref}>
          <Box className={classes.containerColPipeline}>
            <PipelineColumnVirtual
              data={
                USING_FULL_PIPELINE
                  ? dataAllPipeline.dataNotContacted
                  : dataNotContacted
              }
              totalData={
                USING_FULL_PIPELINE
                  ? dataAllPipeline.dataNotContacted.length
                  : dataNotContacted.length
              }
              color="error"
              title="Not Contacted"
            >
          ...
          <Box className={classes.containerColPipeline}>
            <PipelineColumnVirtual
              data={
                USING_FULL_PIPELINE
                  ? dataAllPipeline.dataContacted
                  : dataContacted
              }
              totalData={
                USING_FULL_PIPELINE
                  ? dataAllPipeline.dataContacted.length
                  : dataContacted.length
              }
              color="primary"
              title="Contacted"
            >
          ...
          <Box className={classes.containerColPipeline}>
            <PipelineColumnVirtual
              data={
                USING_FULL_PIPELINE ? dataAllPipeline.dataShowing : dataShowing
              }
              totalData={
                USING_FULL_PIPELINE
                  ? dataAllPipeline.dataShowing.length
                  : dataShowing.length
              }
              color="secondary"
              title="Showing"
          ...
          <Box className={classes.containerColPipeline}>
            <PipelineColumnVirtual
              data={
                USING_FULL_PIPELINE
                  ? dataAllPipeline.dataContract
                  : dataContract
              }
              totalData={
                USING_FULL_PIPELINE
                  ? dataAllPipeline.dataContract.length
                  : dataContract.length
              }
              color="success"
              title="Contract"
          ...
          <Box className={classes.containerColPipeline}>
            <PipelineColumnVirtual
              ...
              totalData={
                USING_FULL_PIPELINE
                  ? dataAllPipeline.dataASk.length
                  : dataASk.length
              }
              color="info"
              title="Ask Referrals"
